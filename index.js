require('./config')
const { default: ElisaBotMdConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./thiva/exif')
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('./thiva/myfunc')
...
async function startElisaBotMd() {
    const ElisaBotMd = ElisaBotMdConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['Thivanka Nirmal','Safari','1.0.0'],
        auth: state
    })
    ...
    let halal = await ElisaBotMd.sendContact(callerId, global.owner)
    ...
    ElisaBotMd.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./thiva")(ElisaBotMd, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
//messages.reaction
ElisaBotMd.ev.on('group-messages.reaction', async (anu) => {
        console.log(anu)
        
        })
//messages.delete        

 ElisaBotMd.ev.on('messages.delete', async (anu) => {
        console.log(anu)
    ...
    // Add Other
      ...
      * @param {*} options
      */
    /* ElisaBotMd.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
      let mime = '';
      let res = await axios.head(url)
      mime = res.headers['content-type']
      if (mime.split("/")[1] === "gif") {
     return ElisaBotMd.sendMessage(jid, { video: await getBuffer(url), caption: caption, gifPlayback: true, ...options}, { quoted: quoted, ...options})
      }
      let type = mime.split("/")[0]+"Message"
      if(mime === "application/pdf"){
     return ElisaBotMd.sendMessage(jid, { document: await getBuffer(url), mimetype: 'application/pdf', caption: caption, ...options}, { quoted: quoted, ...options })
      }
      if(mime.split("/")[0] === "image"){
     return ElisaBotMd.sendMessage(jid, { image: await getBuffer(url), caption: caption, ...options}, { quoted: quoted, ...options})
      }
      if(mime.split("/")[0] === "video"){
     return ElisaBotMd.sendMessage(jid, { video: await getBuffer(url), caption: caption, mimetype: 'video/mp4', ...options}, { quoted: quoted, ...options })
      }
      if(mime.split("/")[0] === "audio"){
     return ElisaBotMd.sendMessage(jid, { audio: await getBuffer(url), caption: caption, mimetype: 'audio/mpeg', ...options}, { quoted: quoted, ...options })
      }
      }
*/
    /** Send List Messaage
     ...
     * @returns 
     */
        ElisaBotMd.send5ButMsg = (jid, text = '' , footer = '', but = []) =>{
        ...
        ElisaBotMd.sendMessage(jid, templateMessage)
        }
     ...
     * @returns
     */
    ElisaBotMd.send5ButVid = async (jid , text = '' , footer = '', vid, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ video: vid }, { upload: ElisaBotMd.waUploadToServer })
        var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
     ...
     * @returns
     */
    ElisaBotMd.send5ButGif = async (jid , text = '' , footer = '', gif, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ video: gif, gifPlayback: true }, { upload: ElisaBotMd.waUploadToServer })
        var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
        videoMessage: message.videoMessage,
    ...
    ElisaBotMd.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
       ...
       if (options.asSticker || /webp/.test(mime)) {
        let { writeExif } = require('./thiva/exif')
    ...
    * Send Poll
    */
    ElisaBotMd.sendPoll = async (jid, name = '', optiPoll, options = {}) => {
        ...
        options: optiPoll.map(btn => ({
                ...
                return ElisaBotMd.relayMessage(jid, { pollCreationMessage: pollMessage }, { ...options });
                }
                
    
    //send file
    ElisaBotMd.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
        let type = await ElisaBotMd.getFile(path, true)
        ...
        return await ElisaBotMd.sendMessage(jid, {
            ...options,
            caption,
            ptt,
            [mtype]: { url: pathFile },
